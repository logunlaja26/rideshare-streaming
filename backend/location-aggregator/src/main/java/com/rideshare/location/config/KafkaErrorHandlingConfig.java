...
/**
 * Dead Letter Queue (DLQ) error handling configuration for Kafka consumers.
 *
 * Design decisions:
 ...
 *
 * Why DLQ?
 * - Prevents message loss when processing fails persistently (e.g., Redis down, malformed JSON)
...
public class KafkaErrorHandlingConfig {
    ...
    public DeadLetterPublishingRecoverer deadLetterPublishingRecoverer(
        ...
        DeadLetterPublishingRecoverer recoverer = new DeadLetterPublishingRecoverer(
            ...
            (consumerRecord, exception) -> {
                log.error("Message processing failed after retries - sending to DLQ. " +
     ...
     * - Gives transient failures (network blip, Redis restart) time to recover
    ...
    public CommonErrorHandler kafkaErrorHandler(
        ...
        errorHandler.setRetryListeners((record, ex, deliveryAttempt) -> {
            log.warn("Retry attempt {} for message: topic={}, partition={}, offset={}, key={}. Error: {}",
                deliveryAttempt,
                record.topic(),
                record.partition(),
                record.offset(),
        ...
        log.info("Kafka error handler configured with exponential backoff (3 retries) and DLQ topic: {}",
            DLQ_TOPIC);
