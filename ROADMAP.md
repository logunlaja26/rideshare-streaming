...
- [ ] Install Java 21 (Temurin), Maven, Docker Desktop, kubectl, Helm, Terraform CLI, AWS CLI `[infra]`
- [ ] Create GitHub repo `rideshare-streaming` with `/backend`, `/frontend`, `/infrastructure` folders `[ci/cd]`
- [ ] Write `docker-compose.yml`: Kafka (KRaft mode), Redis, Postgres, Kafka UI on port 8080 `[infra]`
- [ ] Run `docker compose up` — verify Kafka UI at localhost:8080, Postgres on 5432, Redis on 6379 `[infra]`
- [ ] Create Spring Boot parent POM with dependency management: spring-kafka, JPA, Redis, Actuator, Lombok `[backend]`
- [ ] Bootstrap 6 empty Spring Boot modules: gps-producer, location-aggregator, trip-event-service, surge-pricing-engine, notification-service, api-gateway `[backend]`
- [ ] Create S3 bucket + DynamoDB table for Terraform remote state (one-time manual AWS step) `[infra]`
- [ ] Set up AWS OIDC identity provider + IAM role for GitHub Actions — no long-lived access keys `[ci/cd]` `[infra]`
